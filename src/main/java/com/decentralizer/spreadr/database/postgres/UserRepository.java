...
interface UserRepository extends R2dbcRepository<UserDBRow, UUID> {
    ...
    @Query("select * from docker.user u where u.name ilike %:name%")
    Flux<UserDBRow> getByName(String username);

    @Query("select * from docker.user u where u.surname ilike %:surname%")
    Flux<UserDBRow> getBySurname(String surname);

    @Query("select * from docker.user u where u.login ilike :login and password like :password")
    Mono<UserDBRow> getByLoginAndPasswordEncrypted(String login, String password);

    @Query("select * from docker.user u where u.login ilike :login")
